...
DAILY_JOKE_RANGE_END = datetime.date(2026, 7, 31)
...
# Optional per-date override for the daily broadcast message (instead of a
# random pick from JOKES). Fill these in day by day for 28.07-03.08 as you
# come up with them - any date left out just uses a random joke as usual.
DATE_SPECIFIC_MESSAGES = {
    datetime.date(2026, 7, 28): "Сьогодні на один день ближче до зустрічі 😊",
    datetime.date(2026, 7, 29): "Сьогодні рівно тиждень до зустрічі 🗓️",
    datetime.date(2026, 7, 30): "Твій останній день пити та гуляти — наступні дні набираємось сил 🍻",
    datetime.date(2026, 7, 31): "Сьогодні та завтра треба гарно виспатися 😴",
}
...
# Same building as Sergey, different floor/apartment.
ARSHAK_MAKSYM_ADDRESS = "Carrer de Parcerisa, 25, 2nd floor, 8th apartment, Sants-Montjuïc, 08014 Barcelona, Spain"
...
PROGRAM = {
    "uk": {
        ...
        "9": "🏠 Йдемо додому та чекаємо подальших інструкцій",
    },
    "hy": {
        ...
        "9": "🏠 Գնում ենք տուն և սպասում ենք հետագա հրահանգների",
    },
    "pl": {
        ...
        "9": "🏠 Idziemy do domu i czekamy na dalsze instrukcje",
    },
}
...
TEXTS = {
    "uk": {
        ...
        "info_arshak_maksym_address": "🏠 Адреса, де живуть Аршак і Максим: {address}",
    ...
    "pl": {
        ...
        "info_arshak_maksym_address": "🏠 Adres, gdzie mieszkają Arshak i Maksym: {address}",
    ...
    "hy": {
        ...
        "info_arshak_maksym_address": "🏠 Հասցեն, որտեղ ապրում են Արշակը և Մաքսիմը՝ {address}",
...
async def start_in_group_redirect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """If someone runs /start in the group chat, don't start the flow there
    (it would show a reply-keyboard to the whole group) - just point them to DM."""
    user = update.effective_user
    await update.message.reply_text(
        t(user.id, "group_welcome").format(bot_username=context.bot.username)
    )
...
async def maybe_finalize(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in arrivals:
        ...
        for day in ("6", "7", "8", "9"):
            lines.append(f"📅 {t(user_id, 'date_label_template', d=day)}")
        ...
        lines += [
            ...
            t(user_id, "info_arshak_maksym_address", address=ARSHAK_MAKSYM_ADDRESS),
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(t(update.effective_user.id, "cancel_msg"), reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
async def testall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=chat_id, text="📅 До зустрічі залишилось N дн.")
    ...
    for day in ("6", "7", "8", "9"):
        agenda_lines.append(f"📅 {t(user.id, 'date_label_template', d=day)}")
    ...
    agenda_lines += [
        ...
        t(user.id, "info_arshak_maksym_address", address=ARSHAK_MAKSYM_ADDRESS),
...
async def daily_broadcast(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in PARTICIPANT_IDS:
        if send_text:
            ...
            if today in DATE_SPECIFIC_MESSAGES:
                message = f"{countdown}\n\n{DATE_SPECIFIC_MESSAGES[today]}"
            else:
                message = countdown
...
def main():
    ...
    app.add_handler(CommandHandler("start", start_in_group_redirect, filters=~filters.ChatType.PRIVATE))

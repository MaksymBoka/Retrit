...
5) /sos          - asks the person to share their location, then alerts
...
6) Daily broadcast (countdown + joke) at a different random time each day,
   ...
   UK); 5 Aug - group voice (ES then UK); 6 Aug - group Polish voice +
   Ukrainian text. Group ones fall back to individual DMs if GROUP_CHAT_ID
   isn't set.
...
SETUP:
  ...
  6) Run the bot, have everyone DM /myid, then fill PARTICIPANT_IDS,
     PARTICIPANT_LANG (uk/ru/hy per person), and PARTICIPANTS (with each
...
async def testall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_tts_voice(context, chat_id, AUG4_VOICE_TEXT_ES, "es")
    await send_tts_voice(context, chat_id, AUG4_VOICE_TEXT_UK, "uk")

    await context.bot.send_message(chat_id=chat_id, text="🧪 ТЕСТ: 5 серпня (група)")
    await send_tts_voice(context, chat_id, AUG5_VOICE_TEXT_ES, "es")
    await send_tts_voice(context, chat_id, AUG5_VOICE_TEXT_UK, "uk")

    await context.bot.send_message(chat_id=chat_id, text="🧪 ТЕСТ: 6 серпня (група)")
    await send_tts_voice(context, chat_id, AUG6_VOICE_TEXT_PL, "pl")
    await context.bot.send_message(chat_id=chat_id, text=AUG6_TEXT_UK)
...
async def aug4_individual_voice_job(context: ContextTypes.DEFAULT_TYPE):
    """4 August - individual voice reminder to each person, Spanish then Ukrainian."""
    for user_id in PARTICIPANT_IDS:
        await send_tts_voice(context, user_id, AUG4_VOICE_TEXT_ES, "es")
        await send_tts_voice(context, user_id, AUG4_VOICE_TEXT_UK, "uk")


async def aug5_group_voice_job(context: ContextTypes.DEFAULT_TYPE):
    """5 August - group voice message, Spanish then Ukrainian (falls back to
    individual DMs if GROUP_CHAT_ID isn't set)."""
...
async def aug6_group_job(context: ContextTypes.DEFAULT_TYPE):
    """6 August - group message: Polish voice + Ukrainian text (falls back
    to individual DMs if GROUP_CHAT_ID isn't set)."""
    targets = [GROUP_CHAT_ID] if GROUP_CHAT_ID else PARTICIPANT_IDS
    for chat_id in targets:
        await send_tts_voice(context, chat_id, AUG6_VOICE_TEXT_PL, "pl")
        try:
            ...
            logging.warning(f"Could not send Aug 6 text to {chat_id}: {e}")
